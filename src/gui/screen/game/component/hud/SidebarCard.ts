...
interface SidebarCardProps extends UiComponentProps {
  ...
  strings: any;
  persistentHoverTags?: { value: boolean };
}
...
export class SidebarCard extends UiComponent<SidebarCardProps> {
  ...
  updateTooltip(item: any, container: any): void {
    const tooltip = resolveSidebarItemTooltipText(
      item,
      this.props.sidebarModel,
      this.props.strings,
    );
  ...
  createTextBox(text: string, color: string, additionalStyle?: any): any {
    ...
    if (typeof text === "string" && text.includes("\n")) {
      ...
      const lineHeight = Math.ceil(
        capHeight.actualBoundingBoxAscent + capHeight.actualBoundingBoxDescent || fontSize * 1.2,
      );
      ...
      for (let index = 0; index < lines.length; index += 1) {
        context.fillText(
          lines[index],
          centerX + 0.5,
          topY + index * (lineHeight + lineSpacing) + 0.5,
          maxTextWidth,
        );
      }

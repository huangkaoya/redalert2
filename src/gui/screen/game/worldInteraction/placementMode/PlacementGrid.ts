...
export class PlacementGrid {
  ...
  private refreshRangeCircle(): void {
    ...
    if (!this.lastRangeCircle || rangeIndicator.radius !== this.lastRangeCircle.radius) {
      const rangeObject = OverlayUtils.createGroundCircle(
        rangeIndicator.radius * Coords.getWorldTileSize(),
  ...
  private getTileOverlayTexture(): THREE.Texture {
    ...
    for (let rampType = 0; rampType < rampHeights.length; rampType++) {
      ...
      context.moveTo(
        -tileOrigin.x + first.x,
        -tileOrigin.y + first.y + (1 - heights[0]) * halfTileHeight + 2 * rampType * screenTileSize.height,
      );
      ...
      for (let cornerIndex = 1; cornerIndex < corners.length; cornerIndex++) {
        ...
        context.lineTo(
          -tileOrigin.x + screen.x,
          -tileOrigin.y + screen.y + (1 - heights[cornerIndex]) * halfTileHeight + 2 * rampType * screenTileSize.height,
        );
      }

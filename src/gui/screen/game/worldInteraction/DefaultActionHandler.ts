...
class SelectAction {
  ...
  constructor(
    ...
    private readonly toggleSelect: boolean = false,
  ) {}
...
export class DefaultActionHandler {
  ...
  static factory(
    ...
    const handler = new DefaultActionHandler(
      renderableManager,
      currentPlayer,
      audioVisualRules,
      map.tileOccupation,
  ...
  constructor(
    private readonly renderableManager: any,
    private readonly currentPlayer: any,
    private readonly audioVisualRules: any,
    private readonly tileOccupation: any,
  ) {}
  ...
  private updateMostSignificantAction(
    selected: any[],
    hover: any,
    target: any,
    filter: ActionFilter,
    ...
    return actions.reduce((best: any, entry: any) => {
      ...
      const currentBeatsBest =
        specialActions.includes(entry.action) ||
  ...
  execute(
    ...
    if (allowed) {
      if (
        this.mostSignificantAction instanceof MoveOrder ||
        ...
        this.renderableManager.createTransientAnim(this.audioVisualRules.moveFlash, (renderable: any) => {
          renderable.setPosition(
            Coords.tile3dToWorld(
              target.tile.rx + 0.5,
              target.tile.ry + 0.5,
              target.tile.z + (target.getBridge()?.tileElevation ?? 0),
            ),
          );
        });

...
export class PlanningMode {
  ...
  constructor(
    ...
    private readonly messageList: any,
    ...
    private readonly targetLines: any,
    private readonly maxWaypointPathLength: number,
  ) {}
  ...
  enter(): void {
    ...
    this.waypointLines = new WaypointLines(
      this.unitSelection,
      this.player,
      this.selectedPaths,
      this.paths,
      ...
      if (path.original) {
        ...
        if (path.original.units.length === 0) {
          ...
          path.waypoints = path.waypoints.filter(
            (waypoint: any) => waypoint.draft || path.original.waypoints.includes(waypoint.original),
          );
        }

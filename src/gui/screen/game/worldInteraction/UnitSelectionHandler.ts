...
export class UnitSelectionHandler {
  ...
  private boxSelectOrigin?: { x: number; y: number };
  ...
  constructor(
    ...
    private readonly unitSelection: any,
    private readonly entityIntersectHelper: any,
    private readonly veteranCap: number,
  ) {
  ...
  selectMultipleUnits(
    units: any[],
    meta: { queryType?: QueryType; veteranLevel?: number; healthLevel?: number } = {},
  ...
  startBoxSelect(pointer: { x: number; y: number }): void {
  ...
  updateBoxSelect(pointer: { x: number; y: number }): void {
  ...
  finishBoxSelect(pointer: { x: number; y: number }, clearExisting: boolean): boolean {
  ...
  selectByVeterancy(): void {
    ...
    const candidates = this.vetNavSelectionSet.filter(
      (unit) => unit.rules.selectable && !unit.isDestroyed && !unit.isCrashing && !unit.limboData && unit.owner === owner,
    );
  ...
  selectByHealth(): void {
    ...
    const candidates = this.healthNavSelectionSet.filter(
      (unit) => unit.rules.selectable && !unit.isDestroyed && !unit.isCrashing && !unit.limboData && unit.owner === owner,
    );
  ...
  private clampPointerToWorldViewport(pointer: { x: number; y: number }): { x: number; y: number } {
    const viewport = this.worldScene.viewport;
    return {

...
export class CameraPanHandler {
  ...
  private initialPan?: { x: number; y: number };
  ...
  constructor(
    ...
    private readonly panRate: any,
    private readonly freeCamera: any,
  ...
  start(pointer: { x: number; y: number }): void {
  ...
  update(pointer: { x: number; y: number }, sticky: boolean): void {
    if (!this.startPos) {
      return;
    }

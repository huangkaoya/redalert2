...
export class WorldViewportHelper {
  ...
  private getScreenPosition(worldPosition: Point3D): THREE.Vector2 {
    ...
    if (camera) {
      ...
      if (Number.isFinite(projected.x) && Number.isFinite(projected.y) && Number.isFinite(projected.z)) {
        return new THREE.Vector2(
          viewport.x + ((projected.x + 1) / 2) * viewport.width,
          viewport.y + ((1 - projected.y) / 2) * viewport.height
        );
      }
    }
    ...
    return new THREE.Vector2(
      screenPos.x - origin.x - pan.x + viewport.x + viewport.width / 2,
      screenPos.y - origin.y - pan.y + viewport.y + viewport.height / 2
    );
  }
}
